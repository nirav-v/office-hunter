        ...
        .prompt([
          {
            ...
            type: "input",
            message:
              "What is the id of the department the role is in?(please enter the corresponding number from th etable above)",
          },
          {
            name: "role",
            type: "input",
...
updateEmployeeRole = () => {
  // first display roles table for the user to see
  db.promise()
    .query(
        ...
        .prompt([
          {
            ...
            type: "input",
            message:
              "Which employee's role do you want to update? Just type their id number from the table above",
          },
          {
            name: "roleID",
            type: "input",
            message: "What is the id of the new role you want to assign? Enter the corresponding id number from the second table above",
          }
        ])
        .then((answers) => {
          const role = JSON.stringify(answers.role);
          db.promise().query(`UPDATE employee 
SET role_id = ${answers.roleID}
...
addEmployee = () => {
  ...
  db.promise()
    ...
    .then(([rows, fields]) => {
      ...
      db.promise()
        .query(
          `SELECT employee.id, concat(manager.first_name, " ", manager.last_name) AS manager
        ...
        .then(() => {
          inquirer
            .prompt([
              {
                ...
                type: "input",
                message:
                  "What id number does the employees role title have? (please enter the corresponding number from the first table above)",
              },
              {
                name: "roleID",
                type: "input",
                message:
                  "What id number does the employees manager have? (please enter the corresponding number from the table above)",
              },
              {
                name: "firstName",
                type: "input",
            ...
            .then((answers) => {
              ...
              db.promise().query(`INSERT INTO employee (first_name, last_name,
              role_id, manager_id)
              VALUES (${firstName}, ${lastName}, ${answers.roleID}, ${answers.managerID})`);
            })
            .then(() => {
              askAction();
            });
...
// ^ first exposure to 'callback hell'
